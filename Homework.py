    ...
    for item in src.iterdir(): # ітеруємось по папці
        if item.is_dir():       # перевіряємо чи це папка
            if item.name in (           # пропускаємо папки у які переміщуємо
                ...
                continue
            sanitize_folder(item, TARGET) # рекурсія
        else:
            move_file(item) # переміщуємо файл
    for folder in src.iterdir():
        if folder.is_dir() and folder.name not in (           # пропускаємо папки у які переміщуємо
                            'pictures',
                            'videos',
                            'documents',
                            'music',
...
if __name__ == '__main__':
    LOCATION = Path(sys.argv[1] if len(sys.argv) > 1 else 'Name')
